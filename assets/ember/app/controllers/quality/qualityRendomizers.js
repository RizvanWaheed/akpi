telenor.QualityRendomizersController = Ember.ArrayController.extend({
	pageName:'qualityRendomizers',
	//itemController: 'rendomizer',
	sortProperties: ['name'],
    ...
    teamleads:[{id:30003544,name:'Shamsa Arif'},{id:30008007,name:'Isma Irshad'},{id:30004217,name:'Fatima Hamid'},{id:33338007,name:'Haleema Fayyaz'}],
    rendomizer:{
    	monitoring_for_id:'',
    	belong_id:'',
    	user_id:'',
    ...
    emailDriverStyle:"display:block",
  	...
  	funcTeamLeadByQualityMonitor:function(){
		var _self  = this;
		var value = this.get('rendomizer.teamlead_id');

		if(typeof value === 'undefined' || value == '' || value <= 0){
			return false;
		}
		var formdata = {id: value};
		_self.get('common').ajaxppRequest('POST', 'api/users_api/teamleadscount?format=json', formdata, 'Yes').then(function (response) {   //newRequest is method of our adapter
		   	_self.set('tlagentcount', response);
		   
		}, function(error){ //handle error
			_self.set('tlagentcount', 0);  
		});
		/*Ember.$.ajax({
            url: 'api/users_api/teamleadscount/?format=json',
            type: 'POST',
            data: {id: value}, //{ complaint:_complaint, caller:_caller, complaintLog:_complaintLog, imageForm:formData },
           	/ * mimeType:"multipart/form-data", contentType: false,//'json', cache : false, processData: false,* /
            success: function(data, textStatus, xhr) {
                self.set('tlagentcount', data);
	...
	}.observes('rendomizer.teamlead_id'),
	...
	/*funcByQualityMonitor:function(){
		...
		this.store.find('emaildrivers',{'for_id':value,'type_id':0}).then(function(driver){
			self.set('rendomizera',driver);
		});
		return;
	}.observes('monitoredBy'),*/
	
});
telenor.RendomizerController = Ember.ObjectController.extend({
	need:['rendomizers'],
