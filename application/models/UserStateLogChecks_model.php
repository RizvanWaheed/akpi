<?php
class UserStateLogChecks_model extends CI_Model {
    ...
    */
	public function __construct()
	{
	 ...
	 public function getMystate($identy){

		$sess_data = $this->session->userdata;
		$id = $sess_data['data_logged_in']['id'];

		$this->db->select('usl.user_state_id, (select name from user_states where id = usl.user_state_id) as name, usl.id, usl.created, usl.modified , uslcs.latest_time')
		->from('user_state_logs as usl')
		->join('user_state_log_checks uslcs', 'uslcs.id = usl.id and uslcs.user_id = usl.user_id')
		->where('usl.user_id', $id)
		->where('usl.id', $identy)
		->order_by('usl.id', 'desc')
		->limit(1);
  ...
  public function getLoginUserStatsLogs($user_id){
    ...
    public function getMyAgentStateLog($id, $role_id=null){
		...
		$ii = " usrs.id = user_state_logs.user_id and usrs.employee_id in (select id from asm.tbl_employees where reporting_to = ".$id.")";
		// if($id == null || $id <= 0){
		// 	// $id = $sess_data['data_logged_in']['id'];
