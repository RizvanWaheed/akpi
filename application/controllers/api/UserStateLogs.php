...
class UserStateLogs extends REST_Controller {
	...
	function index_get($id=null) {
		...
		$employee_id = $sess_data['data_logged_in']['employee_id'];
		...
		if($this->get('id') && $this->get('id') > 0 && $this->get('agent') == 'my'){
		...
		if( $project_id == '1251' || $project_id == 'null' || $project_id == '' || empty($project_id) ){
			$users = $this->UserStateLogs_model->getMyProjectAgentStateLog($employee_id, $role_id);
			// }
			// else{
			// 	$users = $this->UserStateLogs_model->getMyAgentStateLog($employee_id, $role_id);
			// }
		}else{
			$users = $this->UserStateLogs_model->getMyAgentStateLog($employee_id, $role_id);
		}
