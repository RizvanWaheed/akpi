...
class Titles_model extends CI_Model {
    ...
    */
    ...
    public function getAllTitles($id){
		...
		$query = $this->db->get();
		//return $query->result();
		return array('titles' => $query->result_array());
	}
    ...
    public function getTitles($values = null){
	...
	//    $branch = array();
	    ...
	    foreach ($elements as $key => $element) {
	    	...
	    	if (array_key_exists("children", $element)){ //style="padding-left:'.$margin-=10.'px"
	    	    $branch.='<optgroup  label="'.$element['name'].'">';
	    	    $branch.=$this->buildSelectHtml($element['children'],$margin+=15);
	   			$branch.='<optgroup>';
	   		}
	   		else{ //style="padding-left:'.$margin.'px"
