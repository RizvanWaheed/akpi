telenor.QualityGccMonitoringsRoute = Ember.Route.extend(telenor.SecureRoute, {
	controllerName: 'qualityGccMonitorings',
	baseUrl: '',
	/*beforeModel: function() {
		if(Em.isEmpty(this.controllerFor("application").get('modules').findBy("link", "quality.gccMonitorings"))){
            this.transitionTo('/');
        }
    },*/
	model: function () {
	...
	actions: {
		createNewNode: function (obj, status, parent) {
			var _self = this;
			var _controller = _self.controllerFor('qualityGccMonitorings');
		...
		save: function (monitoring) {
			...
			var self = this;
			var controller = self.controllerFor('qualityGccMonitorings');
			...
			//var controller = self.controllerFor('qualityGccMonitorings');
			/*self.store.unloadAll('gccMonitorings');
	    	var newGccMonitorings = self.store.createRecord('gccMonitorings',callactivity);
			newGccMonitorings.save().then(function(){
