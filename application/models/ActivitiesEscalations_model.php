<?php
class ActivitiesEscalations_model extends CI_Model {
	...
	public function save($activitiesEscalations) {
		//
		//return $insert;
		//print_r($activitiesEscalations);
		unset($activitiesEscalations['id']);
		$this->db->where('activity_id', $activitiesEscalations['activity_id']);
		$this->db->update('activities_escalations', array('status'=>0));
		return $this->db->insert('activities_escalations', $activitiesEscalations);
		/*$report = array();
	...
	public function getAllComplaintLog($activity_id) {

		$this->db->select('*');
		$this->db->from('activities_escalations');
		$this->db->where('activity_id', $activity_id);
		$this->db->where('status', '1');
		$this->db->order_by('activities_escalations.created', 'desc');
		...
		return array('activitiesEscalations' => $result);
	}
