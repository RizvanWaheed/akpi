telenor.ComplaintsTrackingsView = Ember.View.extend(telenor.AnimateView, { //telenor.MainView.extend({ 
	templateName: 'complaints/trackings',
	...
	didInsertElement: function () {
		...
		function updateLabel(start, end, label) {
			...
			$("#idTrackingReport").value(start.format('MM/DD/YYYY') + ' - ' + end.format('MM/DD/YYYY'));
		}
		Ember.$('#idTrackingReport').daterangepicker({
			format: 'MM/DD/YYYY', //'YYYY_MM_DD',//
			startDate: start,
			endDate: end,
			autoApply: true,
			//  "opens": "center",
		...
		$(function () {
			Em.run.later(function () {
				...
				$('#tabComplaintTracking a:eq(3)').tab('show');
				$("#complaint-tenter").addClass('in active');
				$(".select2").select2({
					width: '100%'
				});
				_self.get('common').hideOverlay('dashboard-tabs');
				Em.$(".tab-content #complaint-tenter").addClass('in active');
			}, 2000);
			// $("a[data-toggle=tab][href=#daily]").tab("show");
			//   $(".tab-content #map").removeClass('active');
			//		$(".tab-content #complaints_tracks_enter").addClass('in active');
			//	Em.$(".select2").select2({width:'100%'});
		});
		// 	Ember.run.later(function(){
		// 		Ember.$('#tabComplaintTracking > .nav-tabs a:last').tab('show');
		// //		Ember.$('#sub_reason_id').select2();
		// 	}, 200);

		// 	$('#tabComplaintTracking > a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
		// 		var target = $(e.target).attr("href") // activated tab
		// 		alert(target);
		// 	});
		//	Em.$('#tabComplaintTracking > .nav-tabs a:last').tab('show');
