<?php
class AgentData_model extends CI_Model {
	...
	public function saveUsersTasks($id, $status){
		// print_r($id);
		// print_r($status);

		$this->db->where('id', $id);
		return $this->db->update('agent_data', array('status'=> $status));
		//$insert = $this->db->insert_batch('', $data);
	    //return $insert;
	}
	...
	public function findAllByUsersTasks($status=null, $user=null){
		$sess_data = $this->session->userdata;
		$user_id = $sess_data['data_logged_in']['id'];
		$role_id = $sess_data['data_logged_in']['role_id'];
		
		$this->db->select('agent_data.id as id, agent_data.task_id, agent_data.user_id, agent_data.sr_no, agent_data.sr_time
		, agent_data.other, agent_data.status, agent_data.date, bc.name, bc.tat, bc.protected')
		->from('agent_data');
		$this->db->join('business_categories as bc', 'agent_data.task_id = bc.id');//, 'right'
		// $this->db->join('agent_data_category_detail as adcd', 'adcd.agent_data_id = agent_data.id', 'right');
		// 4
		// 5
		// 6
		// 7		
		if($user == 'me' && $role_id == 11){
			$this->db->where('user_id', $user_id);
		}
		if($user == 'me' && ($role_id > 4 && $role_id < 11)){
		...
		if($status!=null){
		 ...
		 foreach($new as $ky => $idx){
			 $this->db->select('*')->from('business_categories');
			 $this->db->join('agent_data_category_detail as adcd', 'adcd.business_category_id = business_categories.id and adcd.agent_data_id = '.$idx['id'], 'left');//agent_data
			 $this->db->where('business_categories.parent_id', $idx['task_id']);
			//  [agent_data_id] => 1
            //                 [business_category_id] => 2453
            //                 [checked] => 1
			// $this->db->where();
			$iquery = $this->db->get();
			if(empty($iquery)){
				$new[$ky]['children'] = array();
			}else{
				$new[$ky]['children'] = $iquery->result_array();
			}
			
		 }
		// echo '<pre>'; print_r($new); exit();
		return $new; //array('agentTasks'=> $new);
		
	}
	public function findAllByUsers($status=null){
		$sess_data = $this->session->userdata;
		$user_id = $sess_data['data_logged_in']['id'];
		$role_id = $sess_data['data_logged_in']['role_id'];
		
		$this->db->select('*')->from('agent_data');
		//->join('agent_data_category_detail as adcd', 'adcd.agent_data_id = agent_data.id', 'right');
		if($role_id != 1){
			$this->db->where('user_id', $user_id);
		}
		if($status!=null){
