<?php
class Callactivities_model extends CI_Model {
	...
	public function getAllCallActivitiesPieGraph($type, $region, $area, $territory) {
		...
		$result = $more->result_array();

		//echo '<pre>';
		//print_r($result);

		$newArray = array('isName' => 0
			, 'haveTime' => 0
			, 'isSmoker' => 0
			, 'isEighteen' => 0
			, 'isMet' => 0
		...
		*/
		...
		foreach ($result as $key => $index) {
			...
			$isMet = 'No';
			//  $index['call_status'] == 2 ||
			if ($index['call_status'] == 1 || $index['call_status'] == 4) {
				if ($index['is_named'] == 1) {
					$isName = 'Yes';
					$newArray['isName']++;
				}
		...
		if ($newArray['totals'] > 0) {

			$sucessfull = $newArray['success_full'] + $newArray['unsuccess_full'];
			$response['call'][0]['name'] = 'Success Full';
			$response['call'][0]['y'] = round(($sucessfull / $newArray['totals']) * 100, 1);

			$response['call'][1]['name'] = 'Wrong Number';
			$response['call'][1]['y'] = round(($newArray['wrong_number'] / $newArray['totals']) * 100, 1);

			$response['call'][2]['name'] = 'Call Drop';
			$response['call'][2]['y'] = round(($newArray['call_drop'] / $newArray['totals']) * 100, 1);

			$response['call'][3]['name'] = 'Number Busy';
			$response['call'][3]['y'] = round(($newArray['number_busy'] / $newArray['totals']) * 100, 1);

			$response['call'][4]['name'] = 'No Answar/ Switch off';
			$response['call'][4]['y'] = round(($newArray['no_answar'] / $newArray['totals']) * 100, 1);

			$response['call'][5]['name'] = 'Scheduled Call';
			$response['call'][5]['y'] = round(($newArray['scheduled_call'] / $newArray['totals']) * 100, 1);
			...
			*/
			...
			$SameName = round(($newArray['isName'] / $sucessfull) * 100, 2);
			$response['name'][0]['name'] = 'Yes';
			$response['name'][0]['y'] = $SameName;
			$response['name'][1]['name'] = 'No';
			$response['name'][1]['y'] = 100 - $SameName;

			$smoker = round(($newArray['isSmoker'] / $sucessfull) * 100, 2);
			$response['smoker'][0]['name'] = 'Yes';
			$response['smoker'][0]['y'] = $smoker;
			$response['smoker'][1]['name'] = 'No';
			$response['smoker'][1]['y'] = 100 - $smoker;

			$eighteen = round(($newArray['isEighteen'] / $sucessfull) * 100, 2);
			$response['eighteen'][0]['name'] = 'Yes';
			$response['eighteen'][0]['y'] = $eighteen;
			$response['eighteen'][1]['name'] = 'No';
			$response['eighteen'][1]['y'] = 100 - $eighteen;

			$met = round(($newArray['isMet'] / $sucessfull) * 100, 2);
			$response['met'][0]['name'] = 'Yes';
			$response['met'][0]['y'] = $met;
			$response['met'][1]['name'] = 'No';
			$response['met'][1]['y'] = 100 - $met;

			//$newArray['totals']
		}
		return $response;
		//print_r($response);
		//$response = array('callstatus' => 0, 'callstatus' => 0 )
		/*foreach ($newArray as $nAkey => $nAindex) {
	...
	public function consumerAgentDateImport($id = null, $fDate = null, $tDate = null) {
		//, $region = null, $area = null, $territory = null) {
		...
		$query = $this->db->select('call_activities.id, call_activities.created, call_activities.call_status, is_named, is_smoker, is_eighteen, is_met, is_remember, have_time, aided, call_activities.ambassador_id, BrandAmbassador.code as bacode, BrandAmbassador.mobileNo1 as bamobile1, BrandAmbassador.name as baname, users.login as aglogin, users.name as agname,consumers.msisdn as cumsisdn, consumers.name as cuname, BrandAmbassador.regionID, BrandAmbassador.areaID, regions.name as region, areas.name as area, territories.name as territory')
			->from('call_activities')
			->join('consumers', 'consumers.id = call_activities.consumer_id', 'left outer')
			->join('users', 'users.id = call_activities.created_by', 'left outer')
			->join('BrandAmbassador', 'BrandAmbassador.brandAmbassadorID = consumers.ambassador_id', 'left outer')
			->join('regions', 'regions.id = BrandAmbassador.regionID', 'right outer') //, 'right outer'
			->join('areas', 'areas.id = BrandAmbassador.areaID', 'right outer')
			->join('territories', 'territories.id = BrandAmbassador.territoryID', 'right outer')
		...
		foreach ($query as $key => $index) {
			...
			$return[] = array('msisdn' => $index['cumsisdn'], 'name' => $index['cuname'], 'Correct Name' => $isName, 'Have Time' => $haveTime, 'Smoker' => $isSmoker, 'More Then Eighteen' => $isEighteen, 'Met Brand Ambassador' => $isMet, 'Remember' => $isRemembered, 'Aided' => $aided, 'Call Status' => $call_status, 'Agent Name' => $index['agname'], 'Agent Login' => $index['aglogin'], 'Ambassador Name' => $index['baname'], 'Ambassador Msisdn' => $index['bamobile1'], 'region' => $index['region'], 'area' => $index['area'], 'territory' => $index['territory'], 'Call Date' => $index['created']);
		}
		return $return;
		echo '<pre>';
		print_r($return);
	...
	public function getConsumerRegionAreaTerritoryImportDtoll($id = null, $fDate = null, $tDate = null, $region = null, $area = null, $territory = null) {
		...
		$query = $this->db->select('call_activities.id, call_activities.call_status, is_named, is_smoker, purchase_offer, video_seen, is_eighteen, is_met, is_remember, have_time, aided, call_activities.ambassador_id, BrandAmbassador.code as bacode, BrandAmbassador.name as baname, consumers.msisdn as cumsisdn, consumers.name as cuname, BrandAmbassador.regionID, BrandAmbassador.areaID')
			->from('call_activities')
			->join('consumers', 'consumers.id = call_activities.consumer_id', 'left outer')
			->join('BrandAmbassador', 'BrandAmbassador.brandAmbassadorID = consumers.ambassador_id', 'left outer')
			->where('consumers.name !=', '')
			->where('BrandAmbassador.regionID', 5)
		//	->where('BrandAmbassador.areaID', $area)
		//	->where('BrandAmbassador.territoryID', $territory)
		//->join('consumers', 'consumers.id = call_activities.consumer_id', 'left outer')
		//->where('BrandAmbassador.status', 'Enabled')
		...
		foreach ($query as $key => $index) {
			...
			if ($index['purchase_offer'] == 1 && $haveTime == 'Yes' && $isSmoker == 'Yes' && $isEighteen == 'Yes') {
				$purchaseOffer = 'Yes';
			} else if ($index['purchase_offer'] == 0 && $haveTime == 'Yes' && $isSmoker == 'Yes' && $isEighteen == 'Yes') {
				$purchaseOffer = 'No';
			}

			if ($index['video_seen'] == 1 && $haveTime == 'Yes' && $isSmoker == 'Yes' && $isEighteen == 'Yes') {
				$videoShown = 'Yes';
			} else if ($index['video_seen'] == 0 && $haveTime == 'Yes' && $isSmoker == 'Yes' && $isEighteen == 'Yes') {
				$videoShown = 'No';
			}
			...
			if ($index['call_status'] == 1) {
				...
				$call_status = 'Scheduled Call Back';
			} else if ($index['call_status'] == 8) {
				$call_status = 'Not Interested';
			} else if ($index['call_status'] == 9) {
				$call_status = 'Dead on Arrival';
			} else if ($index['call_status'] == 10) {
				$call_status = 'Original Customer N/A';
			} else if ($index['call_status'] == 11) {
				$call_status = 'Customer Busy';
			}


			$return[] = array('msisdn' => $index['cumsisdn'], 'name' => $index['cuname'], 'Correct Name' => $isName, 'Have Time' => $haveTime, 'Smoker' => $isSmoker, 'More Then Eighteen' => $isEighteen, 'Met Brand Ambassador' => $isMet, 'Remember' => $isRemembered, 'Aided' => $aided, 'Video Shown' =>  $videoShown, 'Pack Offered' =>  $purchaseOffer, 'Call Status' => $call_status, 'Brand Ambassador Name' => $index['baname']);
		}
	...
	public function queryAllCallActivities($from_date, $to_date, $sSearch, $iDisplayLength, $iDisplayStart, $iSortCol, $iSortDir, $type, $region, $area, $territory) {
	...
	public function getAllCallActivitiesLocation($id = null, $fDate = null, $tDate = null, $type = null, $sSearch = null, $iDisplayLength = null, $iDisplayStart = null, $iSortCol = null, $iSortDir = null, $region = null, $area = null, $territory = null) {
		...
		$query = $this->queryAllCallActivities($from_date, $to_date, $sSearch, $iDisplayLength, $iDisplayStart, $iSortCol, $iSortDir, $type, $region, $area, $territory);
		//print_r($query); exit();
	...
	public function getAllAgentCallActivities($id = null, $fDate = null, $tDate = null, $sSearch = null, $iDisplayLength = null, $iDisplayStart = null, $iSortCol = null, $iSortDir = null, $region = null, $area = null, $territory = null, $report = null) {
		...
		if ($report == 'voq') {
			$cols = array('cumsisdn', 'cuname', 'is_named', 'is_smoker', 'is_eighteen', 'is_met', 'is_remember', 'have_time', 'aided', 'call_status', 'baname', 'bacode');
		} else {
		...
		foreach ($query as $key => $index) {
			...
			$bagname = $index['agname'];
			if ($report == 'voq') {
				$bagname = $index['baname'];
			}
	...
	public function getAllCallActivitiesDtoll($phone = null) {
		$this->db->select('count(call_activities.id) as count, sum(case when consumers.city = "multan" then 1 else 0 end) multan, , sum(case when consumers.city = "lahore" then 1 else 0 end) lahore,
		...
		$this->db->where('BrandAmbassador.regionID', 5);*/
		//consumers.ambassador_id
		//
		//$this->db->where('call_activities.is_smoker', 1);
		//$this->db->where('call_activities.is_eighteen', 1);
		//$this->db->where('call_activities.is_met', 1);
		//$this->db->group_by('consumers.id,  call_status');
		//echo '<pre>';
		/*$query = $this->db->get();
		$result = $query->result_array();
		print_r($result); exit();*/
	...
	public function getAllCallActivitiesDtollVerifiedTerritories($phone = null) {


		$this->db->select(' count(call_activities.id) as count
			, sum(case when have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1  then 1 else 0 end) data, BrandAmbassador.territoryID, territories.name territory_name, users.name as user_name , users.id as user_id');
		$this->db->from('call_activities');
		$this->db->join('consumers', 'consumers.id = call_activities.consumer_id');
		$this->db->join('BrandAmbassador', 'BrandAmbassador.brandAmbassadorID = consumers.ambassador_id', 'left outer');
		$this->db->join('territories', 'territories.id = BrandAmbassador.territoryID', 'right outer');
		$this->db->join('users', 'users.territory_id = BrandAmbassador.territoryID');
		$this->db->where('call_activities.call_status', 1);
		$this->db->where('BrandAmbassador.regionID', 5);
		$this->db->where('BrandAmbassador.status', 'Suspended');
		$this->db->group_by('BrandAmbassador.territoryID, territories.name, users.name, users.id');
		...
		$return = array();
		foreach($query as $key => $index){
			$query[$key]['percentage'] = round(($index['data']/$index['count']*100),1);
		}
		//print_r($query);
		//exit();
		return $query;
	...
	public function getAllCallActivitiesDtollVerified($phone = null) {


		$this->db->select('count(call_activities.id) as count
			, sum(case when consumers.city = "multan" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1  then 1 else 0 end) multan
			, sum(case when consumers.city = "lahore" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1  then 1 else 0 end) lahore
			, sum(case when consumers.city = "karachi" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 then 1 else 0 end) karachi
			, sum(case when consumers.city = "islamabad" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 then 1 else 0 end) islamabad');
		$this->db->from('call_activities');
		$this->db->join('consumers', 'consumers.id = call_activities.consumer_id');
		$this->db->where('call_activities.call_status', 1);
	...
	public function getAllCallActivitiesDtollAided($phone = null) {


		$this->db->select('count(call_activities.id) as count
			, sum(case when consumers.city = "multan" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and aided = 1 then 1 else 0 end) multan
			, sum(case when consumers.city = "lahore" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and aided = 1  then 1 else 0 end) lahore
			, sum(case when consumers.city = "karachi" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and aided = 1 then 1 else 0 end) karachi
			, sum(case when consumers.city = "islamabad" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and aided = 1 then 1 else 0 end) islamabad ');
		$this->db->from('call_activities');
		$this->db->join('consumers', 'consumers.id = call_activities.consumer_id');
		$this->db->where('call_activities.call_status', 1);
	...
	public function getAllCallActivitiesDtollVideo($phone = null) {


		$this->db->select('count(call_activities.id) as count
			, sum(case when consumers.city = "multan" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and video_seen = 1 then 1 else 0 end) multan
			, sum(case when consumers.city = "lahore" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and video_seen = 1  then 1 else 0 end) lahore
			, sum(case when consumers.city = "karachi" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and video_seen = 1 then 1 else 0 end) karachi
			, sum(case when consumers.city = "islamabad" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and video_seen = 1 then 1 else 0 end) islamabad ');
		$this->db->from('call_activities');
		$this->db->join('consumers', 'consumers.id = call_activities.consumer_id');
		$this->db->where('call_activities.call_status', 1);
	...
	public function getAllCallActivitiesDtollPacket($phone = null) {


		$this->db->select('count(call_activities.id) as count
			, sum(case when consumers.city = "multan" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and purchase_offer = 1 then 1 else 0 end) multan
			, sum(case when consumers.city = "lahore" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and purchase_offer = 1  then 1 else 0 end) lahore
			, sum(case when consumers.city = "karachi" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and purchase_offer = 1 then 1 else 0 end) karachi
			, sum(case when consumers.city = "islamabad" and have_time = 1 and is_met = 1 and is_smoker = 1 and is_eighteen = 1 and purchase_offer = 1 then 1 else 0 end) islamabad ');
		$this->db->from('call_activities');
		$this->db->join('consumers', 'consumers.id = call_activities.consumer_id');
		$this->db->where('call_activities.call_status', 1);
	...
	public function updateCallActivity($data) {
		...
		$insert['video_seen'] = isset($data['isVideoUsage'])?$data['isVideoUsage']:'';
		$insert['purchase_offer'] = isset($data['isPurchaseOffer'])?$data['isPurchaseOffer']:'';
