<?php
class ActivitiesAdjustments_model extends CI_Model {
	...
	public function save($activitiesAdjustments) {
		//
		//return $insert;
		//print_r($activitiesAdjustments);
		unset($activitiesAdjustments['id']);
		$this->db->where('activity_id', $activitiesAdjustments['activity_id']);
		$this->db->update('activities_adjustments', array('status'=>0));
		return $this->db->insert('activities_adjustments', $activitiesAdjustments);
		/*$report = array();
	...
	public function getAllComplaintLog($activity_id) {
		...
		$this->db->select('*');
		$this->db->from('activities_adjustments');
		$this->db->where('activity_id', $activity_id);
		$this->db->where('status', '1');
		$this->db->order_by('activities_adjustments.created', 'desc');
		...
		return array('activitiesAdjustments' => $result);
	}
