telenor.QualityQltymonitoringsRoute = Ember.Route.extend(telenor.SecureRoute, {
	controllerName: 'qualityQltymonitorings',
	baseUrl: '',
	/*beforeModel: function() {
		if(Em.isEmpty(this.controllerFor("application").get('modules').findBy("link", "quality.qltymonitorings"))){
            this.transitionTo('/');
        }
    },*/
	model: function () {
	...
	setupController: function (controller, model) {
		...
		controller.set('monitoredByQualityMonitor', 1);
	...
	actions: {
		...
		funcSaveMonitorActivity: function (upselling) {
			...
			var self = this;
			var controller = self.controllerFor('qualityQltymonitorings'); //callStatus
			...
			//var controller = self.controllerFor('qualityQltymonitorings');
			/*self.store.unloadAll('qltymonitorings');
	    	var newQltymonitorings = self.store.createRecord('qltymonitorings',callactivity);
			newQltymonitorings.save().then(function(){
		...
		funcPhoneCallActivity: function () {
			...
			var controller = self.controllerFor("Qltymonitorings");
