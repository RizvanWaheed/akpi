telenor.UploadersAgentDataSheetsRoute = Ember.Route.extend(telenor.SecureRoute, {
	controllerName: 'uploadersAgentDataSheets',

	renderTemplate: function () {
		this.render('uploaders/AgentDataSheets');
	},
	/*beforeModel: function() {
		...
		if(Em.isEmpty(this.controllerFor("application").get('modules').findBy("link", "quality.AgentDataSheets"))){
			this.transitionTo('/');
		}
    },*/
	model: function () {
	...
	clearing: function () {
		var _self = this;
		var _controller = this.controllerFor('uploadersAgentDataSheets');
	...
	actions: {
		uploadCallDataFile: function (Id) {
			...
			_self.get('common').ajaxppRequest('POST', 'api/Uploaders/agentDataSheetsFile?format=json', formData, 'Yes', 'form').then(this.success.bind(this), this.failure.bind(this));

			// var request = Ember.$.ajax({
			//     url: "api/users_api/uploadAgentDataSheetsFile",
			//     type: 'POST',
			//     data:  formData,
			//   	//async: false,
