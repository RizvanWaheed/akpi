...
class Datatables extends REST_Controller {
	...
	function uploadInOutTimesFile_get($id = null) {
		//print_r($this->get('id'));
		//	print_r($id);
		exit();
...
*/
	...
	function accalations_post() {
		...
		if($id){
			...
			if ($this->AccalationLogs_model->save($accalationLog)) {
				$this->response(array('success' => 'Escalation Saved succesfully !'), 200); // 200 being the HTTP response code
			} else {
	...
	function myMgmtComplaint_get($return = null) {
		//if ($return != null) {
			$complaints = $this->Complaints_model->getMyMgmtComplaintCount();
	...
	function myComplaint_get($return = null) {
		if ($return != null) {
			$complaints = $this->Complaints_model->getMyComplaintsCount();
			$this->response($complaints, 200);
		} else {			
			$complaints = $this->Complaints_model->getMyComplaint($this->get('sSearch'), $this->get('iDisplayLength'), $this->get('iDisplayStart'), $this->get('iSortCol_0'), $this->get('sSortDir_0'), $this->get('from_date'), $this->get('to_date'), $this->get('report'));
	...
	function mycomplaints_get($return = null, $ph = null) {
		if ($return != null) {
			...
			$complaints = $this->Complaints_model->getAllMyComplaints($this->get('start'), $this->get('end'), $this->get('complaint_date'), $this->get('complaint_status'), $this->get('complaint_sub_department'));
	...
	function complaints_get($return = null, $ph = null) {
		if ($return != null) {
			$complaints = $this->Complaints_model->getComplaintsCount($this->get('phone'), $this->get('field'));
			$this->response($complaints, 200);
		} else {
			$start = $this->get('start');
			$end = $this->get('end');
			...
			//}
			//else{
			$complaints = $this->Complaints_model->getAllComplaints($this->get('start'), $this->get('end'), $this->get('phone'), $this->get('field'));
			//}
			if ($complaints) {
				$this->response($complaints, 200); // 200 being the HTTP response code
			} else {
...
//		print_r($categories);
	...
	function roles_get($id = null) {
		...
		if ($roles) {
			$this->response($roles['roles'], 200); // 200 being the HTTP response code
		} else {
			$this->response(array('error' => 'Couldn\'t find any role!'), 404);
		}
	}
