telenor.QualitySheetUploadsRoute = Ember.Route.extend(telenor.SecureRoute, {
	controllerName: 'qualitySheetUploads',

	renderTemplate: function () {
		this.render('quality/sheetUploads');
	},
	/*beforeModel: function() {
		...
		if(Em.isEmpty(this.controllerFor("application").get('modules').findBy("link", "quality.sheetUploads"))){
			this.transitionTo('/');
		}
    },*/
	model: function () {
	...
	clearing: function () {
		var _self = this;
		var _controller = this.controllerFor('qualitySheetUploads');
	...
	actions: {
		uploadCallDataFile: function (Id) {
			...
			formData.set("monitoring_for_id", _controller.get('monitoring_for_id'));

			// formData.append(file, frm);
			var request = Ember.$.ajax({
				url: "api/users_api/uploadCallsFile",
				type: 'POST',
				data: formData,
				//async: false,
