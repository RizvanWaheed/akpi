telenor.ComplaintsRegistrationsRoute = Ember.Route.extend(telenor.SecureRoute, {
    controllerName: 'complaintsRegistrations',
    baseUrl: '',
   /* pageSize: 20,
    pageNumber: 1,
    pageSizeNew: 20,
    ...
    /*beforeModel: function() {
        if(Em.isEmpty(this.controllerFor("application").get('modules').findBy("link", "complaints.registrations"))){
            this.transitionTo('/');
        }
    },*/
    model: function() {
       ...
       /* var baseUrl = this.controllerFor("application").get("applicationURL");
        ...
        self.store.find('monitoringCategories', {'monitoring_for_id':this.get('campaign').mapBy('id')}).then(function(drivers){
		   ...
		   //$(window).load(function() {
		 	...
		 	Em.$("#complaint-enter").addClass('in active');
    ...
    actions: {
        editview: function(uid) {
            ...
            var _controller = _self.controllerFor('complaintsRegistrations');
        ...
        findNumber: function() {
           ...
           /* Ember.$.isLoading({
            ...
            complt =  { rtrn: 'single', phone: _complaint.id, field:'id', start: 1, end: 20 };
        ...
        findTicket: function() {
            ...
            complt =  { rtrn: 'single', phone: _complaint.ticket_id, field:'ticket_id', start: 1, end: 20 };
            this.send('findType', complt);
           // var store = _self.store;
        ...
        findBooking: function() {
           ...
           /* Ember.$.isLoading({
            ...
            complt =  { rtrn: 'single', phone: _complaint.booking_id, field:'booking_id', start: 1, end: 20 };
            this.send('findType', complt);
           // var store = _self.store;
        ...
        findMsisdn: function() {
            ...
            complt =  { rtrn: 'all', phone: _complaint.msisdn, field:'msisdn', start: 1, end: 20 };
            this.send('findType', complt);
        ...
        findEmail: function() {
           ...
           /* Ember.$.isLoading({
            ...
            complt =  { rtrn: 'single', phone: _caller.email, field:'email', start: 1, end: 20 };
            this.send('findType', complt);
           // var store = _self.store;
        ...
        reset: function() {
            ...
            //this.controllerFor("complaintsRegistrations").set('complaintLogs', logComplaint);
        ...
        //    var self = this;
        //    var controller = self.controllerFor('complaintsRegistrations');
            if (_complaint.id == 0) {
        ...
        loadMoreNew: function() {
            ...
            var self = this;
            var controller = this.controllerFor("complaintsRegistrations");
        ...
        selectPageNew: function(number) {
            ...
            if (!this.get('visitedPageNew').contains(number)) {
                ...
                //this.controllerFor("complaintsRegistrations").set('model', mod);
                //	this.controllerFor("Complaints").get('arrangedContent').push(mod);
            } else {

                //this.controllerFor("complaintsRegistrations").set('model',this.get('model').slice(start, end));
                //this.controllerFor("complaintsRegistrations").get('arrangedContent').slice(start, end);;
            }
            //  console.log(this.store.all('complaints').get('content'));
            this.controllerFor("complaintsRegistrations").set('pageNew', number);
            //this.set('page', number);
        },
        ...
        delete: function(uid) {
            ...
            newComplaint.save().then(function() {
                ...
                self.controllerFor("complaintsRegistrations").set('model', userJson);
                self.send('reloadAll');
                 _self.get('common').hideLoader();
            });
