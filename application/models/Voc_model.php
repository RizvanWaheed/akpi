<?php
class Voc_model extends CI_Model {
	...
	public function save($data){
		$insert = $this->db->insert('voc', $data);
	    return $insert;
	}
	public function saveMany($data){
		$insert = $this->db->insert_batch('voc', $data);
	    return $insert;
	}
	public function findAll($user_id=null){
		...
		// print_r($new); exit();
		return array('voc'=> $new);
		
	}
	public function findKey($from_time, $to_time){

		$this->db->select('id')->from('voc')->where('from', $from_time)->where('to', $to_time)->limit(1);
		$query 	= $this->db->get();
		$new 		= $query->row_array();
		// print_r($new); exit();
		return $new['id'];
