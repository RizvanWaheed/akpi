<?php
class VocSetups_model extends CI_Model {
	...
	public function saveMany($data){
		$insert = $this->db->insert_batch('user_shifts', $data);
	    return $insert;
	}
	public function findAll(){

		$this->db->select('*')->from('voc_setup');
		 $query = $this->db->get();
		 $new =  $query->result_array();
		// print_r($new); exit();
		return array('voc_setup'=> $new);
		
	}
	public function findKey($from_time, $to_time){

		$this->db->select('id')->from('voc_setup')->where('from', $from_time)->where('to', $to_time)->limit(1);
		$query 	= $this->db->get();
		$new 		= $query->row_array();
		// print_r($new); exit();
		return $new['id'];
